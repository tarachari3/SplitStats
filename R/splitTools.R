...
#' queries <- c('Mouse','Rat')
#' pvals <- calcPval(featMat,labs,queries)
#' @export
#' @importFrom stats fisher.test

# ---------------------- Rank features by p-value for given split ------------------ (FOR PACKAGE)
calcPval <- function(featureMat,labels, queryLabs) {
  ...
  for (i in rollcallNums){
    ...
    #Construct contingency table for each feature relative to split of interest
...
#' @examples
#' featMat <- cbind(c(0,1,1,1,1),c(0,0,0,1,1),c(0,1,1,1,1),c(0,1,0,1,1),c(0,0,0,0,0),c(0,1,1,0,0))
#' ds <- dist(featMat, method = "manhattan",diag=TRUE,upper=TRUE)
#' ds <- as.matrix(ds)
#' labs <- c('Mouse','Fly','Rat','Zebrafish','Human')
#' out <- getSplits(labs, ds)
#' @export
...
#' Get split distances between two groups of observations (e.g. members)
#'
#'
#'
...
#' @param queryLabs : Vector of observation labels on one side of a split (subset of labels)
#' @param refLabs : Vector of reference observations labels (subset of labels)
...
#' @author Tara Chari
#'
#' @param labels : Vector of labels for observations (e.g. members) in dataset
...
#' splits <- out[2][[1]]
#' cycle <- out[1][[1]]
#' getSplitVis(labs,cycle,splits,ds,'','')
#' @export
#' @importFrom reticulate import
#' @importFrom basilisk basiliskStart basiliskRun basiliskStop
